...
const files=['manifest.json','index.js','style.css',...walk(path.join(root,'src')).filter(p=>/\.(js|css|html)$/.test(p)).map(p=>path.relative(root,p).replaceAll('\\','/'))].filter(p=>p!=='src/build-info.js').sort();
