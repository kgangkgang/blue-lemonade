...
export function mixControls(s,{slider,esc}) {
    ...
    return `<div class="salty-group bl-mix"><div class="salty-row"><span>에이드 혼합하기<small>${mode==='light'?'라이트':'나이트'} 전용 · 2~3가지 에이드를 섞어요. 글자는 따로 골라요.</small></span><button type="button" class="salty-btn" data-act="mix-toggle" aria-pressed="${m.on}">${m.on?'혼합 끄기':'에이드 혼합하기'}</button></div>${m.on?`<div class="bl-mix-swatches" role="group" aria-label="섞을 에이드">${Object.entries(PALETTE_FAMILIES).filter(([k])=>k!=='custom').map(([key,f])=>`<button type="button" data-act="mix-family" data-family="${key}" aria-pressed="${m.families.includes(key)}" ${!m.families.includes(key)&&m.families.length===3?'disabled':''}><i style="background:${safeColor(paletteColors({...s,palette:paletteVariant(key,mode)}).accent)}"></i>${esc(f.label)}${m.families.includes(key)?` <b>${m.families.indexOf(key)+1}</b>`:''}</button>`).join('')}</div>${slider(path+'.angle','혼합 방향 (°)',0,360,1,90)}${m.families.map((f,i)=>slider(`${path}.weights.${i}`,`${PALETTE_FAMILIES[f].label} 비중`,1,100,1,50)).join('')}<p class="salty-note">최대 3색 · 색 비중은 서로의 상대적인 넓이예요. 180° 돌리면 색 순서가 반대가 돼요. 색 고치기에서 항목별로 단색도 고를 수 있어요.</p>`:''}</div>`;
}
