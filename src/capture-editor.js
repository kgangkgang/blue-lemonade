...
export function editCaptureDraft(ids,current){
 return new Promise((resolve,reject)=>{
  ...
  const render=()=>{list.innerHTML=draft.map((m,i)=>`<section data-edit-message="${i}"><h4>메시지 #${m.id}</h4>${m.blocks.map((b,j)=>`<article data-edit-block="${j}"><label>문단 ${j+1}${b.media?.length?` · ${b.media.some(x=>x.video)?'영상':'이미지'}`:''}${b.media?.length?`<span class="bl-capture-media">${b.media.map(x=>`<span>${x.src&&!x.video?`<img src="${esc(x.src)}" alt="" loading="lazy">`:''}<small>${esc(x.name||'이름 없는 그림')}</small></span>`).join('')}</span>`:''}<textarea rows="${b.media?.length&&!b.text.trim()?1:3}" placeholder="${b.media?.length?'그림은 그대로 들어가요. 글을 적으면 그림과 같이 나와요.':''}" aria-label="메시지 ${m.id} 문단 ${j+1}">${esc(b.text)}</textarea></label><div class="bl-word-actions"><button type="button" class="salty-btn" data-edit-move="-1" ${j===0?'disabled':''}>↑ 위로</button><button type="button" class="salty-btn" data-edit-move="1" ${j===m.blocks.length-1?'disabled':''}>↓ 아래로</button><button type="button" class="salty-btn" data-edit-delete>문단 삭제</button><button type="button" class="salty-btn" data-edit-only>이 문단만</button></div></article>`).join('')}<button type="button" class="salty-btn" data-edit-add>＋ 문단 추가</button></section>`).join('');};
