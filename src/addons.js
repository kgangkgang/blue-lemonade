...
async function startOne(id){
    try{
        ...
        if(id==='direction'){
            ...
            module=await import('./addons/notes/index.js');
        }else if(id==='models'){
...
export async function startAddons(){
    ...
    const ids=[];
    for(const id of ['perf','order','models','modelswitch','regexlink','rewrite','direction','bookmarks','notes','assets','translator','prompt','customstyle','words']){
...
export function addonMarkup(s,id,nested=false){
    ...
    const on=!!s.addons[id],needsReload=['order','perf','models','modelswitch','regexlink','rewrite','direction','bookmarks','notes','assets','translator','prompt','customstyle'].includes(id)&&on!==running.has(id);
    ...
    const guideBadge=id==='notes'?`<button type="button" class="bl-addon-ver" data-notes-guide title="메모 사용방법" aria-label="메모 사용방법">v${NOTES_VERSION}</button>`:'';  // 1.2.0: 한글화 패널처럼 버전 버튼 = 자세한 사용방법
    const header=`<header class="bl-addon-header"><div><h3><i class="fa-solid ${icons[id]}" aria-hidden="true"></i> ${names[id]}${guideBadge}</h3><p role="status" class="bl-addon-status ${on?'is-on':''}">${status}</p></div><label class="bl-addon-power"><span>기능 켜기</span><input type="checkbox" data-addon-toggle="${id}" ${on?'checked':''} ${saving?'disabled':''}></label></header>`;
    ...
    const notesMenu=id==='notes'?`<label class="bl-addon-toggle"><input type="checkbox" data-addon-menu="notes" ${s.addonUI.notesMenu!==false?'checked':''}><span>요술봉에 메모 표시</span></label><label class="bl-addon-toggle"><input type="checkbox" data-addon-menu="notesBar" ${s.addonUI.notesBarMenu!==false?'checked':''}><span>입력창 위에 메모 줄 표시 (빠른 답장처럼 · 제목을 누르면 내용)</span></label>`:'';
    ...
    return `<div class="bl-addon-layout bl-addon-embedded-layout"><div class="bl-addon-main">${header}${toolSection(id+'-activation','실행·설정',actions,true)}${where||wand||modelMenu||notesMenu?toolSection(id+'-position','미리보기',where+wand+modelMenu+notesMenu,true):''}${position||perf?toolSection(id+'-menus','표시할 위치',position+perf,true):''}</div><div class="bl-addon-config bl-addon-inline" data-addon-inline="${id}"><h3 class="bl-inline-title">세부 설정</h3><div class="bl-addon-inline-content" role="region" aria-label="${names[id]} 세부 설정"><p class="salty-note">${running.has(id)?'설정을 불러오는 중…':'기능을 켜고 새로고침하면 여기에서 설정할 수 있어요.'}</p></div></div>${credit}</div>`;
}
...
export function bindAddons(root,refresh){
    ...
    root.querySelectorAll('[data-addon-open]').forEach(button=>button.addEventListener('click',async()=>{
        ...
        if(button.dataset.addonOpen==='notes'){(await import('./addons/notes/index.js')).openPanel();return;}
    ...
    root.querySelectorAll('[data-notes-guide]').forEach(button=>button.addEventListener('click',async event=>{event.preventDefault();event.stopPropagation();const open=running.has('notes')?async()=>(await import('./addons/notes/index.js')).openPanel():null;(await import('./addons/notes/guide.js')).showNotesGuide({onOpen:open});}));
}
export async function syncAddonIcons(){
    ...
    if(running.has('notes'))(await import('./addons/notes/index.js')).syncMenu();
