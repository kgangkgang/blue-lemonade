...
const folders={assets:'char-assets',prompt:'prompt-panel',customstyle:'SillyTavern-CustomThemeStyleInputs',translator:'llm-translator-custom',order:'panel-order',perf:'perf-assist',words:'word-replace',models:'model-register',rewrite:'ban-word-rewrite',bookmarks:'chat-bookmarks'};
const names={assets:'캐릭터 에셋',prompt:'한글화 패널',customstyle:'커스텀 CSS 조절',translator:'LLM 번역',order:'확장 순서',perf:'성능 보조',words:'단어 치환',capture:'채팅 캡처',models:'모델 등록',modelswitch:'모델 전환',regexlink:'프롬프트 연동 정규식',rewrite:'다시 쓰기',bookmarks:'북마크'};
const icons={assets:'fa-images',prompt:'fa-table-list',customstyle:'fa-sliders',translator:'fa-language',words:'fa-arrow-right-arrow-left',capture:'fa-camera',order:'fa-arrow-down-short-wide',perf:'fa-gauge-high',models:'fa-circle-plus',modelswitch:'fa-shuffle',regexlink:'fa-link',bookmarks:'fa-bookmark',rewrite:'fa-glasses'};
...
export async function conflict(id){
    ...
    const candidates=id==='assets'?['char-assets','character-assets','esetham']:[folders[id]];
...
export async function startAddons(){
    ...
    for(const id of ['perf','order','models','modelswitch','regexlink','rewrite','bookmarks','assets','translator','prompt','customstyle','words']){
...
export function addonMarkup(s,id,nested=false){
    ...
    const on=!!s.addons[id],needsReload=['order','perf','models','modelswitch','regexlink','rewrite','bookmarks','assets','translator','prompt','customstyle'].includes(id)&&on!==running.has(id);
