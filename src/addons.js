...
const names={order:'확장 순서',perf:'성능 보조',words:'단어 치환',capture:'채팅 캡처',models:'모델 등록',modelorder:'모델 순서',modelswitch:'모델 전환',regexlink:'프롬프트 연동 정규식',rewrite:'다시 쓰기',bookmarks:'북마크'};
const icons={words:'fa-arrow-right-arrow-left',capture:'fa-camera',order:'fa-arrow-down-short-wide',perf:'fa-gauge-high',models:'fa-circle-plus',modelorder:'fa-arrow-down-wide-short',modelswitch:'fa-shuffle',regexlink:'fa-link',bookmarks:'fa-bookmark',rewrite:'fa-glasses'};
...
export async function startAddons(){
    ...
    for(const id of ['perf','order','models','modelorder','modelswitch','regexlink','rewrite','bookmarks','words']){
...
export function addonMarkup(s,id){
    const on=!!s.addons[id],needsReload=['order','perf','models','modelorder','modelswitch','regexlink','rewrite','bookmarks'].includes(id)&&on!==running.has(id);
    ...
    const where=['order','perf'].includes(id)?`<div class="bl-addon-position-preview" aria-label="아이콘 위치 미리보기"><small>확장 관리 탭 상단</small><div><span>확장 프로그램 설치</span>${s.addonUI[id+'Icon']?`<span class="bl-preview-location"><i class="fa-solid ${id==='order'?'fa-arrow-down-short-wide':'fa-gear'}"></i> ${names[id]}</span>`:'<span>아이콘 숨김</span>'}</div></div>`:`<div class="bl-addon-position-preview"><small>사용 흐름</small><div class="bl-model-flow"><span>${id==='rewrite'?'금지 묘사 정하기':id==='modelswitch'?'조합 고르기':'공급자 선택'}</span><span>${id==='rewrite'?'답변 검사':id==='models'?'모델 이름 입력':id==='modelswitch'?'바꿀 확장 체크':'등록한 모델 정렬'}</span><span>${id==='rewrite'?'그 문장만 다시 쓰기':id==='modelswitch'?'바꾸기':'저장'}</span></div></div>`;
...
function bindInlineAddon(editor) {
    ...
    const update=async()=>{
        ...
        try{
            const api=await (id==='order'?import('./addons/order/panel.js'):id==='perf'?import('./addons/perf/hub.js'):id==='models'?import('./addons/models/panel.js'):id==='modelswitch'?import('./addons/modelswitch/index.js'):id==='regexlink'?import('./addons/regexlink/index.js'):id==='rewrite'?import('./addons/rewrite/index.js'):id==='bookmarks'?import('./addons/bookmarks/index.js'):import('./addons/modelorder/index.js'));
