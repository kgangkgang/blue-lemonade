// Adapter for the installed translator's command/API and cache schema.
// No translator credentials or private implementation are bundled here.
import { context, chatKey, enabled, exclusive } from './state.js';
import { paragraphSpan, alignedParagraphs, replaceSpan, jsonAnswer } from './core.js';
const FOLDER = 'third-party/llm-translator-custom';
async function adapter() {
    const { extensionNames } = await import('/scripts/extensions.js');
    if (!extensionNames.includes(FOLDER) || context().extensionSettings.disabledExtensions?.includes(FOLDER)) throw Error('LLM 번역 확장을 켜 주세요.');
    const command = context().SlashCommandParser?.commands?.llmTranslate;
    if (!command?.callback) throw Error('번역 확장의 텍스트 번역 연결을 찾지 못했어요.');
    const module = await import('/scripts/extensions/third-party/llm-translator-custom/index.js');
    if (typeof module.processTranslationText !== 'function') throw Error('지원하는 번역 표시 기능을 찾지 못했어요.');
    return { command, render: module.processTranslationText };
}
...
export function selectionSnapshot() {
    ...
    return { id, message, text, mes: message.mes, display: message.extra?.display_text, swipe: message.swipe_id, chatKey: chatKey() };
}
...
export async function applyTranslation(job) {
    return exclusive(async () => {
        ...
        return { ...job, display, translated: translation, result: job.translated.slice(job.span.start, job.span.end), span: { start: job.span.start, end: job.span.start + job.result.length } };
    });
}
