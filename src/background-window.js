...
const TRIGGERS = '#send_but, #option_regenerate, #option_continue, #mes_continue, .swipe_right, .swipe_left, .bl-onehand button, #mes_impersonate';
let mode = 'audio';
...
async function openWindow() {
    if (on && mode === 'audio') { startHum(); clearInterval(humTimer); humTimer = setInterval(() => { if (state.phase === 'done' && Date.now() - state.doneAt > 90000) closeWindow(); }, 5000); return; }
    if (!on || !document.pictureInPictureEnabled || document.pictureInPictureElement) return;
    try {
        ensureVideo(); draw();
        await video.play();
        await video.requestPictureInPicture();
        clearInterval(timer); timer = setInterval(() => { draw(); if (state.phase === 'done' && Date.now() - state.doneAt > 90000) closeWindow(); }, 250); // 끝난 뒤 1분 반은 번역 등을 위해 열어 둔다
...
function onTap(event) { if (on && event.target?.closest?.(TRIGGERS)) { Object.assign(state, { phase: 'working', chars: 0, since: Date.now(), preview: '' }); openWindow(); } }
function onKey(event) { if (on && event.key === 'Enter' && !event.shiftKey && event.target?.id === 'send_textarea') { Object.assign(state, { phase: 'working', chars: 0, since: Date.now(), preview: '' }); openWindow(); } }
...
export function syncBackgroundWindow(enabled, how = 'audio') {
    ...
    try {
        ...
        listen('GENERATION_STARTED', () => { if (state.phase !== 'working') Object.assign(state, { phase: 'working', chars: 0, since: Date.now(), preview: '' }); });
        listen('STREAM_TOKEN_RECEIVED', (text) => { if (typeof text === 'string') { state.chars = text.length; state.preview = text; } });
