...
function tidyFlags(s) {
    ...
    if (isObj(s.chat)) {
        ...
        s.chat.numComma = flag(s.chat.numComma, true); // 5.3.7 큰 숫자는 쉼표로 (numcomma.js) — 예전 설정에는 없으니 켬
...
export function resetSettings(groups = { look: true }) {
    ...
    if (groups.look) {
        ...
        if (old.chat && typeof old.chat === 'object') {
            for (const key of ['triangleFold', 'weatherAutoRest', 'selectPop', 'colorPop', 'numComma', 'streamFade', 'demFold', 'qrFind', 'qrScroll', 'qrRows', 'qrPlace', 'mesPins']) {
