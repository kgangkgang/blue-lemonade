...
// 켜고 끄는 것은 메모리의 정규식 목록(프리셋 · 전역 · 캐릭터)에 disabled 를 쓰는 것뿐 — 프리셋 파일은 사용자가 저장할 때만 바뀐다.
// 원래 상태는 origin 에 적어 두고, 기능을 끄면 되돌린다. 계산은 engine.js (테스트 가능).
...
let root, inlineHost = null, opening = false, timer = 0, lastRows = [], lastNote = '', applied = new Map();
...
/** 계산하고 바뀐 것만 쓴다. 되돌린다: restore=true 면 origin 으로 */
export function sync({ restore = false } = {}) {
    if (!restore && !listened.length && started) listen(); // 껐다가(stop) 다시 켠 경우
    const s = store();
    const enabled = enabledPrompts();
    const scripts = scriptLists().flat().filter(x => x && x.id);
    ...
    if (restore || !enabled) {
        for (const script of scripts) {
            ...
            delete s.origin[script.id]; applied.delete(script.id);
        }
...
function moduleTable() {
    ...
    if (!groups.size) return `<p class="rl-empty">${esc(lastNote || '연결할 모듈 태그가 있는 정규식이 없어요.')}</p>`;
    const s = store();
...
<div class="inline-drawer-content">
  ...
  <label class="rl-keep"><input type="checkbox" data-keep><span><b>최근 메시지에 그 모듈이 있으면 켜 둠</b><small>이전 답에 남은 상태창 · 선택지가 날것으로 보이지 않게</small></span><input type="number" class="text_pole" data-recent min="1" max="200" aria-label="최근 메시지 수"></label>
...
jQuery(() => { mount(); started = true; listen(); schedule(); });
