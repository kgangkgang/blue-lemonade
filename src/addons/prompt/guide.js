...
const chapters = [
 ...
 ['원하는 모습으로 쓰기', `<p>블루레몬에이드 색을 따라가며 테마 선택은 따로 없어요. 원문·번역문 글자 크기는 설정 → 화면에서 조절하세요.</p><p>설정 → 화면의 설명 표시를 끄면 보조 설명을 숨겨요. 버전 버튼의 상세 사용방법은 언제든 다시 열 수 있어요. 확장 탭의 한글화 패널을 펼치면 바로 사용해요. 블루레몬에이드의 세부 설정에서는 별도 패널로 열려요.</p>`],
 ...
 ['출처 · 수정 · 라이선스', `<p>원본 <a href="https://github.com/anon4961/prompt-panel" target="_blank" rel="noopener noreferrer">anon4961 / Prompt Panel</a> · AGPL-3.0. <a href="https://kkangtong.xyz/posts/138394" target="_blank" rel="noopener noreferrer">「깡」의 개인개조+++ 공유본</a> 위에 2026-09-24 블루레몬에이드 화면·도움말·LLM 연결 방식을 추가했어요. 원작자·수정자와 라이선스의 전체 고지는 블루레몬에이드 버전 옆 ⓒ 버튼에서도 확인할 수 있어요.</p><p>이 프로그램은 무보증으로 제공됩니다. AGPL-3.0 조건에 따라 수정·재배포할 수 있으며, 원작자 고지와 해당 조건을 지켜야 합니다. 배포 ZIP의 LICENSE와 수정 가능한 소스를 참고하세요.</p>`],
];
const intro = {
...
export function installGuide({doc,cfg,save,openPanel,closePanel}) {
 ...
 tabs.querySelectorAll('.pt-tab').forEach(tab=>{const icon=doc.createElement('i');icon.className='fa-solid fa-'+icons[tab.dataset.tab];icon.setAttribute('aria-hidden','true');tab.prepend(icon);});
