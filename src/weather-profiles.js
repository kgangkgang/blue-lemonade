...
export const WEATHER_FIELDS=['weatherArtStyle','weatherLevel','weatherOpacity','weatherSize','weatherSpeed','weatherAngle','weatherMotion','weatherSway','weatherSpin','weatherCurvature','weatherOrbitSize','weatherOrbitDirection','weatherColorMode','weatherColor','weatherColor2','weatherShadowStyle','weatherShadowBlur','weatherWaterStyle','weatherWaterArea','weatherSunStyle','weatherStarStyle','weatherFogStyle','weatherFogArea','weatherFogStretch','weatherFogEdge','weatherFogSwell','weatherFogDepth'];
const take=chat=>Object.fromEntries(WEATHER_FIELDS.map(key=>[key,chat[key]]));
export function syncWeatherProfile(chat){
    chat.weatherArtStyle=chat.weatherArtStyle==='anime'?'anime':'real';
    ...
    for(const profile of Object.values(chat.weatherProfiles))if(profile&&typeof profile==='object'){
        profile.weatherArtStyle??='real';profile.weatherColorMode??='auto';profile.weatherColor??='#91cfff';
    }
    if(!WEATHER_MODES.includes(chat.weatherProfileMode)){
        // Migrate shared values once; switching effects afterwards is independent.
        for(const mode of WEATHER_MODES)if(!chat.weatherProfiles[mode])chat.weatherProfiles[mode]={...take(chat),...(mode!==chat.weather?{weatherArtStyle:'real',weatherColorMode:'auto',weatherColor:'#91cfff'}:{})};
    ...
    if(WEATHER_MODES.includes(chat.weather)&&chat.weather!==previous){
        if(!chat.weatherProfiles[chat.weather])chat.weatherProfiles[chat.weather]={...take(chat),weatherArtStyle:'real',weatherColorMode:'auto',weatherColor:'#91cfff'};
