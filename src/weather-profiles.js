...
export const WEATHER_FIELDS=['weatherArtStyle','weatherArtOutline','weatherLevel','weatherOpacity','weatherSize','weatherSpeed','weatherAngle','weatherMotion','weatherSway','weatherSpin','weatherCurvature','weatherOrbitSize','weatherOrbitDirection','weatherColorMode','weatherColor','weatherColor2','weatherShadowStyle','weatherShadowBlur','weatherWaterStyle','weatherWaterArea','weatherSunStyle','weatherStarStyle','weatherFogStyle','weatherFogArea','weatherFogStretch','weatherFogEdge','weatherFogSwell','weatherFogDepth'];
...
export function syncWeatherProfile(chat){
    ...
    for(const profile of Object.values(chat.weatherProfiles))if(profile&&typeof profile==='object'){
        profile.weatherArtStyle??='real';profile.weatherArtOutline??=false;profile.weatherColorMode??='auto';profile.weatherColor??='#91cfff';
    }
    if(!WEATHER_MODES.includes(chat.weatherProfileMode)){
        // Migrate shared values once; switching effects afterwards is independent.
        for(const mode of WEATHER_MODES)if(!chat.weatherProfiles[mode])chat.weatherProfiles[mode]={...take(chat),...(mode!==chat.weather?{weatherArtStyle:'real',weatherArtOutline:false,weatherColorMode:'auto',weatherColor:'#91cfff'}:{})};
