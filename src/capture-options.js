...
export function bindCaptureOptions(root,changed=()=>{}) {
    ...
    const visibility=()=>{
        root.querySelectorAll('[data-video-option]').forEach(el=>el.hidden=!['video','gif'].includes(cfg.format));
...
export async function openCapturePreview(ids, mount = null, selectedIds = () => ids) {
    ...
    async function generate() {
        ...
        try {
            ...
            const capture=options.format==='video'?(await import('./capture-video.js')).captureVideo:options.format==='gif'?(await import('./capture-gif.js')).captureGif:captureMessages;
            ...
            status.textContent=`${result.width} × ${result.height} · 치환 ${result.replaced}곳 · 이름 ${result.hidden}곳 가림${result.weather?' · 날씨 포함':''}${result.duration?' · '+result.duration+'초':''}${result.scrolling?' · 본문 스크롤':''}`;
