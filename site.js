...
const topics = [
  ...
  { id:'text', label:'대사', title:'대사는 또렷하게, 분위기는 그대로.', sub:'형광펜 모양 · 글자 그라데이션 · 프롬프트 색 · 외곽선', cards: [
  ...
  { id:'capture', label:'캡처', title:'글은 그대로, 날씨만 움직이게.', sub:'이미지 · 영상 · GIF · 이름 가리기 · 캡처용 글 편집 · 문단별 분할', cards: [
...
function galleryBlock(topic, device, cards, withHeading){
  ...
  for(const [file,title,description,poster,gif,credit] of cards){
    ...
    if(file.startsWith('410-export.'))wrap.classList.add('export-media');
