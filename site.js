...
const topics = [
{
  ...
  "tall": true,
  "cards": [
    ["521-direction.mp4", "전개 지시 · 다음 장면을 깃펜에", "원하는 전개를 적고 켜면 대화 요청마다 지시가 들어가요. 끄면 내용은 남고 삽입만 멈춰요. 깃펜을 길게 누르면 창 없이 바로 켜고 꺼요. 켜짐은 깃펜의 빛으로 표시합니다.", "521-direction.jpg"],
    ["520-ade-presets.mp4", "캐릭터 에셋 · 그림 프리셋을 따로", "그림을 폴더별로 나누고 프리셋 전체를 켜거나 꺼요. 같은 파일 이름은 켜진 폴더 중 위쪽 것이 우선해요. 시연 그림은 테마 마스코트 에이드와 나이트예요.", "520-ade-presets.jpg"],
    ["520-ade-toggle.mp4", "캐릭터 에셋 · 쓸 그림만 골라요", "그림을 크게 보고 하나씩 껐다 켜요. 끈 그림은 파일을 지우지 않고 AI에게 알려 주는 목록에서 제외해요. 시연 그림은 테마 마스코트 에이드와 나이트예요.", "520-ade-toggle.jpg"],
...
const SHOWN_NOTES=3;
fetch('release-notes.json?v=521', {cache:'no-cache'}).then(r=>{if(!r.ok)throw new Error('notes');return r.json();}).then(notes=>{$('#notes').replaceChildren();
