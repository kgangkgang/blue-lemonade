...
const topics = [
{
  ...
  "sub": "전개 지시부터 그림 관리와 번역까지 실제 사용 흐름을 살펴보세요. 전개 지시는 5.1.1, 에셋은 5.2.0, 번역·한글화 새 기능은 5.0.0, 기존 LLM 위치·탭과 메뉴 순서는 4.9.8 촬영본이에요.",
  "tall": true,
  "cards": [
    ...
    ["520-assets-presets.mp4", "캐릭터 에셋 · 그림 프리셋을 따로", "그림을 폴더별로 나누고 프리셋 전체를 켜거나 꺼요. 같은 파일 이름은 켜진 폴더 중 위쪽 것이 우선해요. 가상 캐릭터와 직접 만든 시연용 그림으로 촬영했습니다.", "520-assets-presets.jpg"],
    ["520-assets-toggle.mp4", "캐릭터 에셋 · 쓸 그림만 골라요", "그림을 크게 보고 하나씩 껐다 켜요. 끈 그림은 파일을 지우지 않고 AI에게 알려 주는 목록에서 제외해요. 가상 캐릭터와 시연용 그림을 사용했습니다.", "520-assets-toggle.jpg"],
...
const SHOWN_NOTES=3;
fetch('release-notes.json?v=520', {cache:'no-cache'}).then(r=>{if(!r.ok)throw new Error('notes');return r.json();}).then(notes=>{$('#notes').replaceChildren();
