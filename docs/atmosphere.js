...
(() => {
  ...
  const looks = {
    clear: {title:'문장과 여백만.',description:'장식 없이도 편안한 대화 화면.',file:'daynight-look-clear',detail:'효과 끔 · 화이트 → 나이트',number:'01 / CLEAR'},
    rain: {title:'조용히 내리는 비.',description:'작고 가는 빗줄기가 배경을 지나가요.',file:'daynight-look-rain',detail:'비 55% · 화이트 → 나이트',number:'02 / RAIN'},
    snow: {title:'작은 눈이 천천히.',description:'동그란 눈송이만 가볍게 흩날려요.',file:'daynight-look-snow',detail:'눈 55% · 화이트 → 나이트',number:'03 / SNOW'}
  };
  ...
  const heroLooks = ['블루 × 민트 · 이름색과 가운데 형광펜', '피치 × 딸기 · 둥근 밑줄', '라벤더 × 블루 · 굵은 글자, 기울인 형광펜', '민트 × 멜론 · 자간과 문단 여백을 넉넉하게'];
  function syncHeroCaption(){heroCaption.textContent=heroLooks[Math.floor((heroVideo.currentTime+.28)/4.2)%4];}
